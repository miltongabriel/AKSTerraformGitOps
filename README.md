...
3. ~~Proper secrets management for the ArgoCD SSH deploy key~~ — done: `bootstrap/02-keyvault` now stores it in Azure Key Vault, and `terraform/02-argocd` reads it via `data "azurerm_key_vault_secret"` instead of `file()`. `subscription_id`/`tenant_id` are still in `dev.tfvars`, which is tracked (see the warning in Deployment step 2) — moving those into the vault (consumed via the External Secrets Operator) is still open.
...
3. ~~Gestão de segredos de verdade para a chave SSH do ArgoCD~~ — feito: `bootstrap/02-keyvault` agora guarda a chave no Azure Key Vault, e `terraform/02-argocd` lê o valor via `data "azurerm_key_vault_secret"` em vez de `file()`. `subscription_id`/`tenant_id` ainda estão no `dev.tfvars`, que é versionado (ver aviso no passo 2 de Deploy) — mover esses valores para o vault (consumidos via External Secrets Operator) segue em aberto.
